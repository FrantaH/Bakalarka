    ...
    if (!isValidOptionString(argument)) {
      ...
      if (isValidLongOptionString(argument)) {
        ...
        if (splitOptionAndValue(argument, option, value)) {
          ...
          key = option2key.find(
                  option)->second; // get the key for the extracted option name
          ...
          // something like '-s 4'
        {
          ...
          if (argument.length() == 1) { // if a single short option
            ...
            key = option2key.find(
                    argument)->second; // get the key for the extracted option name
...
bool ArgvParser::defineOption(const string& _name,
  ...
  if (isDefinedOption(_name)) {
    cerr << "ArgvParser::defineOption(): The option label equals an already defined option."
         << endl;
    return (false);
  }
  ...
  if (_name.length() == 1 && isDigit(_name[0])) {
    cerr << "ArgvParser::defineOption(): Digits as short option labels are not allowd."
         << endl;
    return (false);
  }
...
bool ArgvParser::defineOptionAlternative(const string& _original,
  ...
  if (!isDefinedOption(_original)) {
    cerr << "ArgvParser::defineOptionAlternative(): Original option label is not a defined option."
         << endl;
    return (false);
  }
  ...
  if (_alternative.length() == 1 && isDigit(_alternative[0])) {
    cerr << "ArgvParser::defineOptionAlternative(): Digits as short option labels are not allowd."
         << endl;
    return (false);
  }
